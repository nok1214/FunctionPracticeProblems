...
function findLongestWord(str) {
    ...
    var theLongestWordCount = 0;                                        // declear a new variable to store the longest word, which start with 0;
    var longestWord = ''                                                //set the output value to a string where it will show the longest word in our string;
    for(var i=0; i< splitString.length; i++) { 
        if(splitString[i].length > theLongestWordCount) {               // check if first word inside splitString's length is greather than the previous value stored inside theLongestWord(0) variable;
            theLongestWordcount = splitString[i].length;                // if is true, theLongestWord will have a new value;
...
console.log(findLongestWord('Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.'));// output 'consectetur'
...
function findLongestWord(str){
    ...
    splitString.forEach(function (word){                                //use .forEach() on splitString variable since that is where we are looking for the longest word; set "word" as .forEach() value;
        if(word.length > theLongestWordCount){                          //use "if" statement to check if word.length is greater than variable theLongestWord which start with 0;
            theLongestWordCount = word.length;                          // if is true, theLongestWord will have a new value;
